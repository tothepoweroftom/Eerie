...
scene.background = assets.get("sky");
  ...
  gltfLoader.load('./models/gltf/Mode-01-web/mode-01-web.gltf', (gltf) => {
    ...
    root.traverse((node) => {
      if (node.isMesh) {
        ...
        node.material.envMap = assets.get("sky")
        // node
    ...
    frameArea(boxSize*0.33, boxSize, boxCenter, camera);
  ...
  gltfLoader.load('./models/gltf/Debris-Rock/debris-rock-web.gltf', (gltf) => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      particle.scale.setScalar(Math.random() * 0.25 + 1.05)
  ...
  function render(mv, mouth) {
    ...
    if (mv) {
      ...
      if (mv.detected > 0.01) {
        objectGroup.position.lerp(new THREE.Vector3(-mv.x * 3, mv.y * 3, 0), 0.23)
        parentDebrisGroup.position.lerp(new THREE.Vector3(-mv.x * 3, mv.y * 3, 0), 0.23)
      ...
      animationMixer.timeScale = easeOutExpo(audioManager.getAudioLevel() ) + 0.2
    }
